            ...
            <div class="space-y-2">
                ...
                <div class="panel max-h-44 overflow-y-auto divide-y" style="border-color: var(--cmm-border);">
                  @for (p of ponttaResults(); track p.id) {
                    <button
                      ...
                      [style.background]="form().projetistaid === p.id ? 'color-mix(in srgb, var(--cmm-accent) 8%, var(--cmm-panel))' : 'transparent'"
                    >
              ...
              @if (form().projetistaid) {
                <div class="panel panel-pad text-xs" style="color: var(--cmm-muted); padding: 0.75rem 1rem;">
                  Projetista: <span class="font-medium" style="color: var(--cmm-ink);">{{ form().name || 'Selecionado' }}</span>
                  <span class="font-mono ml-2">{{ form().projetistaid }}</span>
                </div>
              }
            </div>
